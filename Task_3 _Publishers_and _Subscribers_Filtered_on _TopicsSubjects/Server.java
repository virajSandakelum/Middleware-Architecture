...
public class Server {
    ...
    public void start(int port) {
        try {
            serverSocket = new ServerSocket(port); // handle the communication with client, perform on the client's
                                                   // socket
        ...
        while (true) {
            try {
                Socket clientSocket = serverSocket.accept(); // accept incoming client connection and establish a socket
    ...
    private class ClientHandler implements Runnable {
        ...
        public void run() {
            try {
                out = new PrintWriter(clientSocket.getOutputStream(), true); // associated with the client's output
                                                                             // stream(send the data to the client over
                                                                             // the socket connection)
                in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream())); // associated with the
                                                                                               // client's input
                                                                                               // stream(read the data
                                                                                               // to the client over the
                                                                                               // socket connection)

                String inputLine = in.readLine();
                System.out.println("Client" + id + " is a " + inputLine.toLowerCase());
                isPublisher = inputLine.equalsIgnoreCase("PUBLISHER"); // determine the client is a publisher or
                                                                       // subscriber
